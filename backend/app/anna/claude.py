...
# ── Provider-agnostic layer (Claude default; OpenAI alternative) ─────────────
OPENAI_URL = "https://api.openai.com/v1/chat/completions"
PROVIDERS = ["anthropic", "openai"]

DEFAULT_MODELS = {
    "anthropic": MODELS,
    "openai": {"fast": os.environ.get("ANNA_OPENAI_FAST_MODEL") or "gpt-4o-mini",
...
def provider_model(provider: str = "anthropic", tier: str = "fast", override: str | None = None) -> str:
    if override:
        return override
    return DEFAULT_MODELS.get(provider, DEFAULT_MODELS["anthropic"]).get(tier) or MODELS["fast"]


async def llm_complete(*, provider: str = "anthropic", api_key: str | None = None, model: str | None = None,
                       system=None, messages: list, tools=None, max_tokens: int = 1024) -> dict:
    """Uniform completion across providers -> {text, toolCalls:[{name,input}], raw}."""
    if not api_key:
        raise RuntimeError("No API key configured")
    if provider == "openai":
        return await _openai_complete(api_key, provider_model("openai", "fast", model), system, messages, tools, max_tokens)
    out = await call_claude(api_key=api_key, model=provider_model("anthropic", "fast", model),
                            max_tokens=max_tokens, system=system, messages=messages, tools=tools)
    raw = out["raw"]
    tool_calls = [{"name": b.get("name"), "input": b.get("input") or {}}
                  for b in (raw.get("content") or []) if b.get("type") == "tool_use"]
    return {"text": out["text"], "toolCalls": tool_calls, "raw": raw}


async def _openai_complete(api_key, model, system, messages, tools, max_tokens):
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        res = await client.post(OPENAI_URL, headers={"authorization": f"Bearer {api_key}",
                                                     "content-type": "application/json"}, json=body)
    if not res.is_success:
        raise RuntimeError(f"OpenAI API error {res.status_code}: {res.text[:300]}")
    raw = res.json()
