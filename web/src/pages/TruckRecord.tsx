...
export default function TruckRecord() {
  ...
  const operator = s.drivers.find((d) => d.id === t.operatorDriverId);

  return (
    <>
      <PageHeader crumbs={[{ label: 'Carriers' }, { label: s.currentCarrier.name }, { label: 'Trucks', to: `/carriers/${t.carrierId}/trucks` }, { label: `Unit #${t.unit}` }]}
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {tab === 'Details' ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <Card title="Ownership" rows={[['Ownership type', t.ownership === 'company' ? 'Company' : 'Owner Operator'], ['Company (MC)', `${s.currentCarrier.name} · ${t.mc}`], ['Owner', t.owner ?? '—'], ['Operator', operator?.name ?? '—']]} />
          </div>
